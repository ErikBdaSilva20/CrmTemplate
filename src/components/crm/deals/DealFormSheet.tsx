    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full overflow-y-auto sm:w-[480px] sm:max-w-[480px]">
        <SheetHeader>
          ...
          <SheetDescription>
            {editing ? 'Atualize os dados do negócio' : 'Preencha os dados do novo negócio'}
          </SheetDescription>
        </SheetHeader>
        <div className="mt-6 space-y-4">
          ...
          <div className="rounded-md border border-border bg-muted/30 p-3 text-xs text-muted-foreground space-y-1">
            <p>
              <strong className="text-foreground">Valor</strong>: quanto esse negócio representa em
              receita.
            </p>
            <p>
              <strong className="text-foreground">Probabilidade</strong>: chance estimada (%) de
              fechar, usada para priorizar o funil.
            </p>
            <p>
              <strong className="text-foreground">Fechamento</strong>: mês/ano em que esse negócio
              deve fechar (fechamento recorrente).
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select
              value={form.stage_id || ''}
              onValueChange={(v) => onFormChange({ stage_id: v })}
            >
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) => onFormChange({ contact_id: v === 'none' ? null : v })}
            >
              <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              onValueChange={(v) => onFormChange({ company_id: v === 'none' ? null : v })}
            >
              <SelectTrigger>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => onFormChange({ probability: Number(e.target.value) })}
              />
            </div>
            <div className="space-y-2">
              ...
              <MonthYearSelect
                ...
                onChange={(v) => onFormChange({ close_date: v })}
              />
            </div>
          </div>
          <Button onClick={onSave} className="w-full">
            {editing ? 'Salvar' : 'Criar Negócio'}
          </Button>
        </div>
      </SheetContent>
    </Sheet>

...
export function CSVImportModal({ open, onOpenChange, onImported, entityType }: CSVImportModalProps) {
  ...
  // owner_id/org_id NUNCA são enviados — o gateway seta owner_id pela sessão e não há org.
  const handleImport = async () => {
    ...
    try {
      ...
      if (entityType === "contacts") {
        await Promise.all(valid.map((r) => createContact(r as unknown as ContactInsert)));
      } else {
        await Promise.all(valid.map((r) => createCompany(r as unknown as CompanyInsert)));
      }
